// crypto
// node 내장 모듈
const crypto = require('crypto'); //내장 모듈이라 설치 필요X
...
const creatHashPassword = (password) => {
  // createHash(알고리즘).update(암호화할 값).digest(인코딩 방식)
  return crypto.createHash('sha512').update(password).digest('base64'); // .update(암호화하고 싶은 것).digest(인코딩방식)
};

// 해쉬 함수의 한계 : 레인보우 테이블
// => 암화화된 비번을 빠르게 역추적해서 원본 비번을 찾아낼 수 있음
console.log('1234 == ', creatHashPassword('1234'));
console.log('1234 == ', creatHashPassword('1234')); //같은 pw라면 같은 해쉬값이 나옴
console.log('1235 == ', creatHashPassword('1235'));

////////////////////////////////
// pbkdf2(비밀번호, 솔트, 반복횟수, 키의길이, 알고리즘)
// - 비밀번호 기반 키 도출 함수, 주로 비번 저장시 사용
// - buffer 형식의 데이터를 반환 -> toString() 이용해 문자열로 변화
...
// 비번 암호화
const { salt, hash } = saltAndHashPassword(password); // salt: 솔트값, hash: 해시값 (암호화된 값)
console.log(
