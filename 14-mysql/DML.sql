...
-- 형식: foreign key (열_이름) references 기준_테이블(열_이름) update cascade on delete cascade
create table orders (
...
insert into customer values
	 ('asdf', '강세희', '일본 오키나와', '01033331235', '1970-10-31'),
      ('kiwi', '윤지성', '일본 오키나와', '01033331236', '1970-10-31'),
...
delete from customor where custid = 'happy';
...
select birth from customer where custname != '강해린';
-- 사전순으로 박민지보다 뒤에 위치한 고객의 모든 정보 출력
...
-- select * from customer oder by addr desc where birth >= '2000-01-01'; -- error: 구문 순서 오류
...
select custid, sum(amount), sum(price * amount)
 ...
 /* select custid, sum(amount), sum(price * amount)
 from orders where sum(price * amount) >= 10000
